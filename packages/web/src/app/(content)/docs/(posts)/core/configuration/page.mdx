...
interface DriftConfig {
  ...
  plugins?: DriftPlugin[];    // Array of plugins
...
Plugins can be added to extend Drift's functionality:

```typescript
import { loggerPlugin } from "@drift/logger";
import { cachePlugin } from "@drift/cache";
...
Creating and configuring a custom plugin:

```typescript
const customPlugin = new DriftPlugin({
  name: "customPlugin",
...
2. **Plugin Management**
   ...
   - Configure plugins according to environment
...
- Understand [Plugin Development](../plugins/creating.md)
