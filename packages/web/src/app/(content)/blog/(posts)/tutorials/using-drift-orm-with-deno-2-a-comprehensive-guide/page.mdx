...
2. Create a `deps.ts` file to manage your dependencies:
   ```typescript
   export { Drift } from "https://deno.land/x/drift@v1.0.0/mod.ts";
   ```
...
Deno.test("User CRUD operations", async () => {
  ...
  // Add more assertions for read, update, and delete operations
});
```
